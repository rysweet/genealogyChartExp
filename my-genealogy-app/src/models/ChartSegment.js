...
export class ChartSegment {
  ...
  static createFromGeneration(person, generation, position, config) {
    const segmentCount = 2 ** generation;
    const arcAngle = (2 * Math.PI) / segmentCount - (config.arcPadding * Math.PI) / 180;
    const startAngle = position * ((2 * Math.PI) / segmentCount);
    const endAngle = startAngle + arcAngle;
    const innerRadius = config.getInnerRadius(generation);
    const outerRadius = innerRadius + config.getRingWidth(generation);
    ...
    return new ChartSegment({
      ...
      segmentCount,
      arcAngle,
      ringWidth: config.getRingWidth(generation),
      color: config.getColor(person?.id, generation),
      isSelected: config.selectedId === person?.id
    });
  }
  ...
  // New method to generate all text paths for this segment
  getTextPaths(lines) {
    return lines.map((lineText, idx) => {
      const { radius, pathId } = this.getLinePathConfig(idx, lines.length);
      return {
        id: pathId,
        text: lineText,
        d: this.createTextArcGenerator(radius)(),
        transform: this.getTextTransform(radius)
      };
    });
  }

  getLinePathConfig(lineIndex, totalLines) {
    const midRadius = (this.innerRadius + this.outerRadius) / 2;
    const totalHeight = (totalLines - 1) * LINE_SPACING;
    const outermostRadius = midRadius + totalHeight / 2;
    return {
      radius: outermostRadius - lineIndex * LINE_SPACING,
      pathId: `textPath-${this.generation}-${this.position}-line${lineIndex}`
    };
  }
}
